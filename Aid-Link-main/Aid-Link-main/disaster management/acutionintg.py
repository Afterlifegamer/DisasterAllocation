...
def create_auction():
    if request.method == 'POST':
        ...
        new_auction = Auction(material_id=material_id, starting_price=starting_price, current_price=starting_price, end_time=end_time)
...
def place_bid(auction_id):
    ...
    if amount < auction.current_price:
        new_bid = Bid(auction_id=auction_id, bidder_id=bidder_id, amount=amount)
        auction.current_price = amount
        db.session.add(new_bid)
        db.session.commit()
        flash('Bid was placed successfully!')
    else:
        flash('Bid is lower than the current price.')
...
if __name__ == '__main__':
    with app.app_context():
        ...
        print(os.path.abspath('marketplace.db'))
